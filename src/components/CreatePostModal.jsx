...
export const CreatePostModal = ({
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      const created = await uploadPost(postData, imageFile);

      if (created === "AI_PENDING") return;
      onPostCreated();
      onClose();
      cleanupForm();
    } catch (err) {
    ...
    <>
      {showAiModal && (
        <AIWarningModal
          ...
          onCancel={cancelPost}
          onContinue={async () => {
            const done = await overrideAiAndCreatePost();
            if (done === "SUCCESS") {
              onPostCreated();
              onClose();
              cleanupForm();
        ...
        <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 relative">
          ...
          <div className="space-y-3">
            <div
              ...
              data-tip="Enter a descriptive title"
            >
              <input
                type="text"
                placeholder="Title"
